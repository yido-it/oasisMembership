...
$(document).ready(function(){
    ...
    $.fn.hideMenu = function() {$(this).removeClass('menu-active'); $('.menu-hider').removeClass('menu-active');};
	...
	function init_template(){
        ...
        function menuFunctions(){
            ...
            if($('#menu-main').length){
                ...
                $('#'+menuActive).addClass('active-nav bg-highlight');
            }
        ...
        function removeTransitions(){$('body').append('<style id="transitions-remove">.btn, .header, .menu-box, .menu-active{transition:all 0ms ease!important;}</style>');}
            ...
            $('[data-toggle-theme], [data-toggle-theme-switch]').off().on('click',function(){
                ...
                $('body').toggleClass('theme-light theme-dark');
                    ...
                    if($('body').hasClass('theme-light')){
                        ...
                        localStorage.setItem('Eazy-Theme','light-mode')
                    }
                    if($('body').hasClass('theme-dark')){
                        darkSwitch.prop('checked', true);
                        localStorage.setItem('Eazy-Theme','dark-mode')
                    }
                setTimeout(function(){allowTransitions();},150);
                return false;
            })
            if (localStorage.getItem('Eazy-Theme') == "dark-mode") {darkSwitch.prop('checked', true); $('body').removeClass('theme-light').addClass('theme-dark');}
            if (localStorage.getItem('Eazy-Theme') == "light-mode") {darkSwitch.prop('checked', false); $('body').removeClass('theme-dark').addClass('theme-light');}
        ...
        localStorage.setItem('z_Eazy-Visited:' + window.location.pathname, 'Page Visited');
        $.fn.visited = function (className) {
            className = className || false;
            $(this).each(function (index, element) {
                if (element.host == window.location.host &&
                    localStorage.getItem('z_Eazy-Visited:' + element.pathname) ||
                    localStorage.getItem('z_Eazy-Visited:' + element.pathname + '/')) {
                    if (className) {
        ...
        $('.list-group a').on('click',function(){if($(this).attr('href') != '#'){localStorage.setItem('z_Eazy-Visited-Scroll-Position', $(window).scrollTop());}})
        if($('.check-visited').length){
            setTimeout(function(){
                var getScroll = localStorage.getItem('z_Eazy-Visited-Scroll-Position');
                $(window).scrollTop(getScroll)
                 localStorage.removeItem('z_Eazy-Visited-Scroll-Position');
            },200);
        }
        ...
        if(visitDetection.length){
            var lastVisitValue = localStorage.getItem('Eazy-Last-Visited');
            var d = new Date();
            ...
            if(!lastVisitValue){
                $('.never-visited').show();
                $('.before-visited').hide();
                localStorage.setItem('Eazy-Last-Visited', '<br>Your last visit was ' + strDate +' at '+ strTime)
            } else {
       ...
       $(".text-size-default").click(function() {$(".text-size-changer *").css("font-size", "");});
        ...
        $('.search-trending a').on('click',function(){
            ...
            $('.search-box a').removeClass('disabled');
            $('.search-box input').val(search_value);
            $('.search-results').removeClass('disabled');
            $('[data-filter-item]').addClass('disabled');
            $('[data-filter-item][data-filter-name*="' + search_value.toLowerCase() + '"]').removeClass('disabled');
            return false;
        });
        ...
        setTimeout(function(){
            ...
            $('.cover-slider-boxed').owlCarousel({loop:false, margin:18, stagePadding:32, nav:false, lazyLoad:true, items:1, autoplay: true, autoplayTimeout:6000});
        ...
        $('.stepper-add').on('click',function(){
            ...
            if(num <= max){$(this).parent().find('input').val(num)};
            return false;
        });
        ...
        //Stepper Add(1)
        $('.stepper-add-10').on('click',function(){
            var num = +$(this).parent().find('input').val() + 10;
            var max = $(this).parent().find('input').attr('max');
            if(num <= max){$(this).parent().find('input').val(num)};
            return false;
        });
        $('.stepper-sub-10').on('click',function(){
            var num = $(this).parent().find('input').val() - 10;
            if(num >= 0){$(this).parent().find('input').val(num);}
            return false;
        });
        ...
        function highlightLoader(){
            $('[data-highlight-style]').on('click',function(){
                $('[data-highlight-style]').find('.fa').removeClass('fa-check-circle').addClass('fa-circle');
                $(this).find('.fa').removeClass('fa-circle').addClass('fa-check-circle');
                $(this).addClass('menu-active');
                var getHighlight = $(this).data('highlight-style');
                localStorage.setItem('Eazy-Highlight', getHighlight);
                $('#highlight').remove();
                $('head').append( $('<link rel="stylesheet" id="highlight" type="text/css" />').attr('href', getHighlight) );
                return false;
            });
            var rememberHighlight = localStorage.getItem('Eazy-Highlight');
            if (rememberHighlight != null){
                $('[data-highlight-style="'+rememberHighlight+'"]').find('.fa').removeClass('fa-circle').addClass('fa-check-circle');
            }
        ...
        var emailValidator = /^([a-z]+[a-z0-9-_]{3,16})$/;
        var phoneValidator = /^((\+[1-9]{1,4}[ \-]*)|(\([0-9]{2,3}\)[ \-]*)|([0-9]{2,4})[ \-]*)*?[0-9]{3,4}?[ \-]*[0-9]{3,4}?$/;
        var nameValidator = /^/;
        //var nameValidator = /^[가-힣]{2,5}|[a-zA-Z]{2,15}\s[a-zA-Z]{2,15}]/;
        var passwordValidator = /^.*(?=^.{8,15}$)(?=.*\d)(?=.*[a-zA-Z])(?=.*[!@#$%^&*()+=\-_.~]).*$/;
        ...
        //$('.input-required input[type="email"]').on('focusout',function(){if (emailValidator.test($(this).val())){$(this).parent().find('em').html(validIcon);}else{if($(this).val() === ""){$(this).parent().find('em').html("(필수");}else{$(this).parent().find('em').html(invalidIcon);}}});
        $('.input-required input[type="tel"]').on('focusout',function(){if (phoneValidator.test($(this).val())){$(this).parent().find('em').html(validIcon);}else{if($(this).val() === ""){$(this).parent().find('em').html("(필수)");}else{$(this).parent().find('em').html(invalidIcon);}}});
        $('.input-required input[type="password"]').on('focusout',function(){if (passwordValidator.test($(this).val())){$(this).parent().find('em').html(validIcon);}else{if($(this).val() === ""){$(this).parent().find('em').html("(필수)");}else{$(this).parent().find('em').html(invalidIcon);}}});
        $('.input-required input[type="url"]').on('focusout',function(){if (urlValidator.test($(this).val())){$(this).parent().find('em').html(validIcon);}else{if($(this).val() === ""){$(this).parent().find('em').html("(필수)");}else{$(this).parent().find('em').html(invalidIcon);}}});
        $('.input-required input[type="name"]').on('focusout',function(){if (nameValidator.test($(this).val())){$(this).parent().find('em').html(validIcon);}else{if($(this).val() === ""){$(this).parent().find('em').html("(필수)");}else{$(this).parent().find('em').html(invalidIcon);}}});
        $('.input-required textarea').on('focusout',function(){if (textareaValidator.test($(this).val())){$(this).parent().find('em').html(validIcon);}else{if($(this).val() === ""){$(this).parent().find('em').html("(필수)");}else{$(this).parent().find('em').html(invalidIcon);}}});
            ...
            if (isMobile.Android()) {
                function showInstallPrompt(){
                    if($('#menu-install-pwa-android, .add-to-home').length){
                        if (localStorage.getItem('Eazy-PWA-Prompt') != "install-rejected") {
                            setTimeout(function(){
                ...
                $('.pwa-install').on('click',function(e){
                    $('body').addClass('pwa-install-clicked');
